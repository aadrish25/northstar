...
from quart import Quart, websocket, jsonify, request, send_file
...
def require_auth(fn):
    ...
    async def wrapper(*args, **kwargs):
        ...
        logged_user = sessions[token]
        route_user = kwargs.get("user_id")

        # User can only touch their own routes
        if route_user and route_user != logged_user:
            return jsonify({
                "success": False,
                "message": "Forbidden"
            }), 403
...
async def authenticate_websocket():
    ...
    ws://localhost:5000/ws/<user_id>?token=xxxx
    """
...
async def login():
    ...
    if not user_id or password != "demo123":
        return jsonify({
            "success": False,
...
# ----------------------------------
# WebSocket (now protected)
# ----------------------------------

@app.websocket("/ws/<user_id>")
async def ws(user_id):
    authenticated_user = await authenticate_websocket()
...
async def upload_resume(user_id):
    try:
        ...
        file_path = os.path.join(UPLOAD_FOLDER, f"{safe_user_id}_resume.pdf")
...
@require_auth
async def get_resume(user_id):
    path = os.path.join(UPLOAD_FOLDER, f"{user_id}_resume.pdf")
